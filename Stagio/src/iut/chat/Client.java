...
public class Client extends JFrame implements Serializable {
	private String nom;
	// Mot de passe d'un client utilisé par celui ci pour se connceter au
	// serveur
	private String pwd;
	 ...
	 * avec en parametre le nom et le mdp
	 * @param String nom
	 * @param String mdp
	 */
	...
	/**
	 * Fonction qui recupère le mot de passe
	 * @return String pwd
	 */
	...
	/**
	 * Fonction qui recupère l'état 
	 * de connection de l'utilisateur
	 * @return boolean etat
	 */
	...
	/**
	 * Procédure qui affecte un etat de connection de l'utilisateur
	 * @param connected
	 */
	...
	public String toString() {
		...
		if (etat)
			ch = "Conecté";
		return "Nom du client : " + nom + "\nmot de passe du client : " + pwd
				+ " " + ch;
	}


	/**
	 * Procédure permettant l'ajout d'un utilisateur connecté au JList
	 * @param nom
	 */
	public void ajout(String nom) {
		System.out.println("ajout de " + nom
				+ " à la liste des clients connectés");
		noms.add(nom);
	...
	/**
	 * Procedure permettant de supprimer un client qui a déconnecter du JList
	 * @param String nom
	 */
	...
	private void jbInit() throws Exception {
		...
		send.setIcon(new ImageIcon(
				"C:/Documents and Settings/MZT/jbproject/Chat/classes/chat/Images/send.gif"));
	...
	/**
	 * 
	 * Classe interne utilisée pour la gestion des évenements engendrés par les boutons radios
	 *
	 */
	private class GestionBoutonRadio implements ItemListener {
	...
	protected void processWindowEvent(WindowEvent e) {
		if (e.getID() == WindowEvent.WINDOW_CLOSING) {
			if (Usr_Chat.srv_state) {
				try {
					J = (Contrat) Naming.lookup("rmi://" + Usr_Chat.srv_Adr
							+ ":1099/JChat");
			...
			if (!disconnected) {
				try {
					System.out.println("Suppression de l'objet distant " + "//"
							+ Usr_Chat.adr + ":" + Usr_Chat.num_port + "/" + nom);
					Naming.unbind("//" + Usr_Chat.adr + ":" + Usr_Chat.num_port + "/"
							+ nom);
	...
	/**
	 * 
	 * Classe interne utilisée pour la gestion des évenements engendrés par le
	 * bouton Envoyer
	 */
		...
		public void actionPerformed(ActionEvent e) {
			if (destinataires.getSelectedIndex() != -1) {
				...
				Communication com = new Communication(nom,
						(String) noms.get(destinataires.getSelectedIndex()),
						champ.getText());
				try {
					J.envoyer(com);
	...
	synchronized void Disconnect_actionPerformed(ActionEvent e) {
		...
		// distant du client du registre RMI
		if (Usr_Chat.srv_state) {
			try {
				J = (Contrat) Naming.lookup("rmi://" + Usr_Chat.srv_Adr
						+ ":1099/JChat");
			...
			try {
				Naming.unbind("//" + Usr_Chat.adr + ":" + Usr_Chat.num_port + "/" + nom);
